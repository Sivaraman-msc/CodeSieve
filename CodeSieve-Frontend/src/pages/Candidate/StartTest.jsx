...
export default function StartTest() {
  ...
  return (
    <div className="relative w-full min-h-screen bg-white flex items-center justify-center overflow-hidden">
      <img src={s1} alt="background" className="absolute top-0 left-0 w-full h-full object-cover rounded-lg opacity-10 pointer-events-none select-none"/>
      ...
      <div className="relative z-30 w-full max-w-sm bg-white shadow-2xl rounded-xl p-6 mx-4">
        ...
        <form onSubmit={handleClick} className="space-y-5">
          ...
          <button type="submit" className="w-full bg-black text-white py-2 text-sm font-medium rounded hover:opacity-90 transition"> Start Test </button>
        </form>
      </div>
    </div>
  );
